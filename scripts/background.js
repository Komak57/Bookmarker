...
//                         target: { tabId: tabId },
//                         func: getDocumentContent, // The function to be executed in the content script
//                         args: [domain, { id: tab.id, url: tab.url, title: tab.title }, settings]
//                     }, (results) => {
//                         console.log('Script Injection Complete.');
//                         if (results) {
//                             if (results[0]) {
//                                 const documentContent = results[0].result.document;
//                                 console.log('Current Document content retrieved. ', documentContent);

//                                 chrome.scripting.executeScript({
//                                     target: { tabId: results[0].result.tab.id },
//                                     func: MonitorChanges,
//                                     args: [results[0].result.domain, results[0].result.tab, results[0].result.settings, documentContent]
//                                 });
//                             }
//                         }
//                     });
...
// function MonitorChanges(domain, tab, settings, oldDocument) {
//     const observer = new MutationObserver(mutations => {
//         mutations.forEach(mutation => {
//             const documentContent = getDocumentContent(domain, tab, settings).document;

//             // Did we see a change?
//             if (JSON.stringify(documentContent) != JSON.stringify(oldDocument)) {
//                 addEpisodeToStorage(domain, documentContent, tab, settings);
//                 // switch (trackDomain[domain].obtainTitleFrom) {
//                 //     case "URL":
//                 //         addEpisodeToStorage(domain, tab, 1);
//                 //     case "Tab Text":
//                 //         addEpisodeToStorage(domain, tab.title, 2);
//                 //     case "Content on Page":
//                 //         addEpisodeToStorage(domain, tab.url, 1); // Placeholder
//                 //     default:
//                 //         addEpisodeToStorage(domain, tab.url, 1);
//                 // }
...
//     // Optional: Stop observing after a timeout
//     setTimeout(() => {
//         observer.disconnect();
//         console.log('Observation timed out.');
//         // In case we had an updated document before trying...
//         addEpisodeToStorage(domain, oldDocument, tab, settings);
//     }, 5000);
// }
...
function getDocumentContent(settings) {
    ...
    try {
        ...
        if (settings.titleHtmlQueryMatch) {
            ...
            // const sel = document.querySelector(settings.titleHtmlQueryMatch);
            // if (sel)
            //     data.title = sel.innerText.trim();
            // else
            //     data.title = document.body.innerHTML.match(new RegExp(settings.titleHtmlQueryMatch));
        }
        // Example: Get the inner HTML of the body (you can modify this as needed)
        if (settings.seasonHtmlQueryMatch) {
            ...
            // const sel = document.querySelector(settings.seasonHtmlQueryMatch);
            // if (sel)
            //     data.season = sel.innerText.trim();
            // else
            //     data.season = document.body.innerHTML.match(new RegExp(settings.seasonHtmlQueryMatch));
        }
        // Example: Get the inner HTML of the body (you can modify this as needed)
...
// function getEpisodeDetails(data, settings) {
...
//     // Filter the path for more accurate matching
//     let filteredPath = data.replace(yearRegex, "");
//     console.log(`After yearRegex: '${filteredPath}'`);
//     // Find the season
//     const seasonMatch = filteredPath.match(seasonRegex_Rigid);
//     if (seasonMatch)
//         filteredPath = filteredPath.replace(seasonRegex_Rigid, "");
//     console.log(`After seasonRegex_Rigid: '${filteredPath}'`);
...
//         }
//     }
//     // Try to find the season and episode in the URL path
//     //const seasonMatch = path.match(seasonRegex);
//     //const episodeMatch = path.match(episodeRegex);

//     // Extract the season and episode, or default to 1 if not found
//     const season = seasonMatch ? parseInt((seasonMatch[1] ? seasonMatch[1] : seasonMatch[2])) : 1;
...
//     if (eMatch > 0) {
//         // Remove season and episode info from the path to isolate the title
//         cleanPath = data
//             //.replace(seasonRegex, '') // Remove season part
//             .replace((eMatch == 1 ? episodeRegex_Rigid : (eMatch == 2 ? episodeRegex_Simple : episodeRegex_Soft)), '') // Remove episode part
//             .replace(/[-_/]+/g, ' ') // Replace dashes, underscores, and slashes with spaces
//             .trim(); // Clean up any extra spaces
//     }
//     // TODO: Get title from API - https://api.jikan.moe/v4/anime?q=
